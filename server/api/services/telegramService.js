...
export const sendTelegramWithButtons = async (text, buttons) => {
    const markup = {
...
export const notifyNewUser = async (email, userId) => {
    if (process.env.VERCEL) {
...
export const notifyNewInvestment = async (investmentId, userName, amount, userId, invUuid) => {
    if (process.env.VERCEL) {
...
export const notifyDepositSubmitted = async (depositId, userName, amount, method, receiptPath, txHash) => {
    if (process.env.VERCEL) {
...
export const notifyWithdrawalRequest = async (withdrawalId, userName, amount, method) => {
    if (process.env.VERCEL) {
...
export const notifyTradeClosed = async (investmentId, userName) => {
    if (process.env.VERCEL)
        return;
    await sendTelegramMessage(`🔒 Trade Closed\n\nInvestment: ${investmentId}\nUser: ${userName}`);
};
export const notifyAuditLog = async (action, adminId, details) => {
    if (process.env.VERCEL)
        return;
    await sendTelegramMessage(`📋 Audit Log\nAction: ${action}\nAdmin: ${adminId || 'system'}${details ? `\nDetails: ${JSON.stringify(details)}` : ''}`);
};
export const notifyKYCSubmission = async (userId, userName, selfieUrl, idFrontUrl, idBackUrl) => {
    if (process.env.VERCEL) {
